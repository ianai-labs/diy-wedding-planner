        ...
        <AuthenticatedLayout>
            ...
            <div className="flex h-[calc(100vh-8rem)] gap-4">
                ...
                <div className="w-80 shrink-0 bg-white rounded-lg shadow overflow-y-auto">
                    <div className="p-3 border-b font-semibold text-gray-800">Chat Support</div>
                    {usersWithChats.length === 0 ? (
                        <p className="p-4 text-sm text-gray-400 text-center">Belum ada chat.</p>
                    ) : usersWithChats.map(u => (
                        <button key={u.id}
                            onClick={() => loadChat(u)}
                            className={`w-full text-left p-3 border-b hover:bg-gray-50 transition ${selectedUser?.id === u.id ? 'bg-indigo-50 border-l-4 border-l-indigo-600' : ''}`}>
                    ...
                    {!selectedUser ? (
                        ...
                        <>
                            <div className="p-3 border-b font-medium text-gray-800 flex justify-between items-center">
                                ...
                                <Link href={route('admin.users.manage', selectedUser.id)} className="rounded-md bg-indigo-600 px-3 py-1 text-xs font-semibold text-white hover:bg-indigo-500">Kelola Plan</Link>
                            </div>
                            ...
                            <form onSubmit={submitChat} className="p-3 border-t flex gap-2">
                                ...
                                <button type="submit" disabled={processing}
                                    className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500">Kirim</button>
                            </form>
                        </>
                    )}
                </div>
